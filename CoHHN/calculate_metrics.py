"""
Script para calcular métricas de evaluación en sistemas de recomendación.

Este script procesa un archivo JSON con recomendaciones y calcula métricas como Precisión@20, MAP@20, MRR, nDCG@20, Gini Index, Diversidad Agregada, entre otras.
El archivo JSON debe contener una lista de objetos, cada uno con un campo "target" y una lista "recommended".

Uso:
    python calculate_metrics.py ruta/al/archivo_de_recomendaciones.json

Changelog:
    2025-05-29: Se actualizó el script para aceptar la ruta del archivo JSON como argumento de línea de comandos.
    2025-05-29: Se agregaron comentarios descriptivos y un log de cambios al inicio del script.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Calcula métricas de recomendación a partir de un archivo JSON.")
    parser.add_argument("json_path", type=str, help="Ruta al archivo JSON de recomendaciones.")
    args = parser.parse_args()

    json_path = args.json_path
    if not json_path:
        print("Error: Debe proporcionar la ruta al archivo JSON como argumento.")
        sys.exit(1)
