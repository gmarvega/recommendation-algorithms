...
class HyperConv(Module):
    def __init__(self, layers, dataset, emb_size, n_node, n_price, n_category, device=None):
        ...
        # Manejo explícito del dispositivo
        if device is None:
            ...
            raise ValueError("El argumento 'device' debe ser None, str o torch.device")

        self.mat_cp = Parameter(torch.empty(self.n_category, 1, device=self.device_param))
        self.mat_pc = Parameter(torch.empty(self.n_price, 1, device=self.device_param))
        self.mat_pv = Parameter(torch.empty(self.n_price, 1, device=self.device_param))
        self.mat_cv = Parameter(torch.empty(self.n_category, 1, device=self.device_param))
    ...
    def get_embedding(self, adjacency, embedding):
        ...
        i = torch.tensor(indices, dtype=torch.long, device=self.device_param)
        v = torch.tensor(values, dtype=torch.float, device=self.device_param)

        shape = adjacency.shape
        adjacency = torch.sparse_coo_tensor(i, v, torch.Size(shape), device=self.device_param)
        embs = embedding
        item_embeddings = torch.sparse.mm(adjacency, embs)
    ...
    def intra_gate(self, adjacency, mat_v, embedding2):
        ...
        shape = adjacency.shape
        adjacency = torch.sparse_coo_tensor(i, v, torch.Size(shape), device=self.device_param)
        matrix = adjacency.to_dense().to(self.device_param)
...
class CoHHN(Module):
    def __init__(self, adjacency, adjacency_pv, adjacency_vp,adjacency_pc,adjacency_cp,adjacency_cv,adjacency_vc, n_node, n_price, n_category, lr, layers, l2, beta, dataset, num_heads=4, emb_size=100, batch_size=100, device=None):
        ...
        self.HyperGraph = HyperConv(self.layers, dataset, self.emb_size, self.n_node, self.n_price, self.n_category, device=self.device_param)
    ...
    def generate_sess_emb(self, item_embedding, price_embedding, session_item, price_seqs, session_len, reversed_sess_item, mask):
        zeros = torch.zeros(1, self.emb_size, device=self.device_param, dtype=torch.float)
        mask = mask.float().unsqueeze(-1)

        price_embedding = torch.cat([zeros, price_embedding], 0)
        get_pri = lambda i: price_embedding[price_seqs[i]]
        seq_pri = torch.zeros(self.batch_size, list(price_seqs.shape)[1], self.emb_size, device=self.device_param, dtype=torch.float)
        for i in torch.arange(price_seqs.shape[0], device=self.device_param):
            seq_pri[i] = get_pri(i)
        ...
        item_pos = torch.arange(1, seq_pri.size()[1] + 1, device=self.device_param)
        ...
        last_pos_t = torch.where(item_pos - item_last_num >= 0, torch.tensor([1.0], device=self.device_param),
                                 torch.tensor([0.0], device=self.device_param))
        last_interest = last_pos_t.unsqueeze(2).expand_as(sa_result) * sa_result
        price_pre = torch.sum(last_interest, 1)

        item_embedding = torch.cat([zeros, item_embedding], 0)
        get = lambda i: item_embedding[reversed_sess_item[i]]
        seq_h = torch.zeros(self.batch_size, list(reversed_sess_item.shape)[1], self.emb_size, device=self.device_param, dtype=torch.float)

        for i in torch.arange(session_item.shape[0], device=self.device_param):
            seq_h[i] = get(i)
...
def forward(model, i, data):
    ...
    device = model.device_param if hasattr(model, "device_param") else (model._device if hasattr(model, "_device") else torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    session_item = torch.tensor(session_item, dtype=torch.long, device=device)
    session_len = torch.tensor(session_len, dtype=torch.long, device=device)
    price_seqs = torch.tensor(price_seqs, dtype=torch.long, device=device)
    tar = torch.tensor(tar, dtype=torch.long, device=device)
    mask = torch.tensor(mask, dtype=torch.long, device=device)
    reversed_sess_item = torch.tensor(reversed_sess_item, dtype=torch.long, device=device)
...
def train_test(model, train_data, test_data):
    ...
    for i in slices:
        ...
        for K in top_K:
            for prediction, target in zip(index[:, :K], tar):
                if K == 20:  # Solo guardamos la lista top-20 por sesión
                    recommendations_log.append({
                        'target': int(target),
                        'recommended': prediction.tolist()
                    })
    ...
    import json
    with open("output/recommendations_top20.json", "w") as f:
        json.dump(recommendations_log, f, indent=2)
