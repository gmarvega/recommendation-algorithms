'''
===============================================================================
Script para preprocesamiento de datos de reseñas y metadatos de productos de Amazon
===============================================================================

Descripción:
    Este script procesa datos de reseñas y metadatos de productos de Amazon,
    específicamente del dataset 'Grocery_and_Gourmet_Food', para su uso en modelos
    de recomendación basados en sesiones. Realiza limpieza de datos, extracción de
    características (precio, categoría), cálculo de niveles de precios, mapeo de IDs,
    división en conjuntos de entrenamiento/prueba y guardado de los datos procesados
    junto con diversos mapeos útiles y archivos estructurados para análisis y depuración.

 Entradas:
         - `<datasets_name>.json.gz`: Archivo JSON comprimido con reseñas (interacciones usuario-ítem).
           Columnas relevantes: `reviewerID`, `asin`, `unixReviewTime`.
           Ubicación: Directorio especificado en la variable `originalData`.
         - `meta_<datasets_name>.json.gz`: Archivo JSON comprimido con metadatos de productos.
           Columnas relevantes: `asin`, `price`, `category`, `also_buy`, `also_view`.
           Ubicación: Directorio especificado en la variable `originalData`.
    
     Salidas (generadas en `output/Amazon/<datasets_name>/`):
         - `item_mapping_table.csv`: Tabla con mapeo de ítems y sus propiedades (ASIN, precio procesado, categoría final, etc.).
         - `train.txt`: Datos de entrenamiento procesados (secuencias de ítems, precios, categorías y etiquetas) serializados con pickle.
         - `test.txt`: Datos de prueba procesados (secuencias de ítems, precios, categorías y etiquetas) serializados con pickle.

         - `mapping_dicts.pkl`: Diccionarios de mapeo (ej: `reviewerID` a `sessionID`, `asin` a `itemID`, `category` a `categoryID`, etc.) serializados con pickle.
         - `item_mapping_table_with_sequences.csv`: Tabla `item_mapping_table.csv` extendida, indicando en qué secuencias de entrenamiento aparece cada ítem.
         - `categoryID_to_category_mapping.csv`: Mapeo de los IDs numéricos de categorías a sus nombres originales.
         - `itemID_to_asin_mapping.csv`: Mapeo de los IDs numéricos internos de ítems a sus ASIN originales.
         - `priceLevel_summary.csv`: Resumen estadístico de los niveles de precio definidos (conteo, min, max, media, std).
         - `structured_sessions.csv`: Sesiones de entrenamiento en formato CSV estructurado para análisis (sessionID, secuencias de ítems, precios, categorías).
         - `structured_test_sessions.csv`: Sesiones de prueba en formato CSV estructurado.
    
Versión:
    1.0 (basado en la creación original de Kun, Septiembre 2021, con adiciones
    posteriores para guardado de mapeos extendidos y datos estructurados).

Uso:
    Ejecutar como un script de Python. Asegurarse de que los archivos de dataset
    originales (`<datasets_name>.json.gz` y `meta_<datasets_name>.json.gz`) estén en
    la ruta especificada por la variable `originalData`. El script generará archivos
    procesados en el directorio `output/Amazon/<datasets_name>/`.

Referencias:
    - https://github.com/xiaxin1998/DHCN
    - https://github.com/Wang-Shuo/Neural-Attentive-Session-Based-Recommendation-PyTorch/blob/master/main.py

Changelog:
    - 2023-01-01: Adición de guardado de mapeos extendidos y datos estructurados para análisis y depuración.
    - 2025-05-26: Adición de guardado de conteos estadísticos resumidos en 'summary_counts.json'.
'''

import gzip
from collections import defaultdict
from datetime import datetime
import os
import time
import pickle
import pandas as pd
import numpy as np
import json as json

datasets_name = 'Grocery_and_Gourmet_Food'
price_level_num = 2
time_interval = 60*60*24
originalData = "../../datasets/amazon/"

def parse(path):
    g = gzip.open(path, 'rb')
    for l in g:
        yield json.loads(l)

def getDF(path):
    i = 0
    df = {}
    for d in parse(path):
        df[i] = d
        i += 1
    return pd.DataFrame.from_dict(df, orient='index')

###  userID-itemID-timestamp
data_path = originalData + datasets_name + '.json.gz'
# df = getDF('originalData/Tools_and_Home_Improvement.json.gz')
df = getDF(data_path)
interaction = df[['reviewerID', 'asin', 'unixReviewTime']]

###  itemID-price-category-also_buy-also_view-brand

data_path = originalData + 'meta_' + datasets_name + '.json.gz'
df_item = getDF(data_path)

item_property = df_item[['asin', 'price', 'category', 'also_buy', 'also_view']]

def is_number(s):
    try:
        float(s)
        return True
    except ValueError:
        pass
 
    try:
        import unicodedata
        unicodedata.numeric(s)
        return True
    except (TypeError, ValueError):
        pass
 
    return False


def reg_price(price):
    price_num = price.replace('$','')
    if is_number(price_num):
        results = float(price_num)
    else:
        results = ''
    return results

def reg_category(cate):
    results = ''
    if isinstance(cate,list):
        if len(cate) == 0:
            results = ''
        else:
            results = cate
    else:
        results = ''
    return results

item_property['price_num'] = item_property.price.map(reg_price)
item_property['category'] = item_property.category.map(reg_category)

# delete items without price
item_property = item_property[(item_property['price_num'] != '')]
# delete items without category
item_property = item_property[(item_property['category'] != '' )]
# delete item without pric
item_property[['price_num']] = item_property[['price_num']].astype(float)

item_property.drop_duplicates(subset=['asin'],keep='first',inplace=True)

def get_cate(cate_list):
    return cate_list[-1]
# the last (fine-grained) category of an item is viewed as its category
item_property['cate'] = item_property.category.map(get_cate)
item_all = item_property[['asin', 'price_num', 'cate', 'also_buy', 'also_view']]


group_cate_num = pd.DataFrame(item_all.groupby(item_all['cate']).count())
group_num=group_cate_num.reset_index()[['cate','asin']].rename(columns={'asin':'count'})

group_cate_min = pd.DataFrame(item_all['price_num'].groupby(item_all['cate']).min())
group_min=group_cate_min.reset_index()[['cate','price_num']].rename(columns={'price_num':'min'})

group_cate_max = pd.DataFrame(item_all['price_num'].groupby(item_all['cate']).max())
group_max=group_cate_max.reset_index()[['cate','price_num']].rename(columns={'price_num':'max'})



group_cate_mean = pd.DataFrame(item_all['price_num'].groupby(item_all['cate']).mean())
group_mean=group_cate_mean.reset_index()[['cate','price_num']].rename(columns={'price_num':'mean'})


group_cate_std = pd.DataFrame(item_all['price_num'].groupby(item_all['cate']).std())
group_std=group_cate_std.reset_index()[['cate','price_num']].rename(columns={'price_num':'std'})


item_data1 = pd.merge(item_all, group_num, how='left', on = 'cate')
item_data2 = pd.merge(item_data1, group_min, how='left', on = 'cate')
item_data3 = pd.merge(item_data2, group_max, how='left', on = 'cate')
item_data4 = pd.merge(item_data3, group_mean, how='left', on = 'cate')
item_data5 = pd.merge(item_data4, group_std, how='left', on = 'cate')

item_data = item_data5[item_data5['count']>9]
item_data = item_data[item_data['std']!=0]



import math

def logistic(t, u, s):
    gama = s * 3**(0.5) / math.pi
    results = 1 / (1 + math.exp((t-u)/gama))
    return results
def get_price_level(price, p_min, p_max, mean, std):
    # fenzi = math.log(price) - math.log(p_min)
    # fenmu = math.log(p_max) - math.log(p_min)
    # math.e math.pi math.exp(x)
    if std == 0:
        print('only one sample')
        return -1
    fenzi = logistic(price, mean, std) - logistic(p_min, mean, std)
    fenmu = logistic(p_max, mean, std) - logistic(p_min, mean, std)
    if fenmu == 0 or price==0:
        return -1
    results = int(fenzi / fenmu * price_level_num) + 1
    return results

item_data['price_level'] = item_data.apply(lambda row: get_price_level(row['price_num'], row['min'], row['max'], row['mean'], row['std']), axis=1)
item_final = item_data[item_data['price_level'] != -1]
item_final = item_final[['asin', 'price_num', 'cate','also_buy', 'also_view', 'price_level']]

# Export item mapping table to CSV
#item_final.to_csv('CoHHN/' + datasets_name + '/item_mapping_table.csv', index=False)
item_mapping_table_path = os.path.join('output/Amazon', datasets_name, 'item_mapping_table.csv')
item_final.to_csv(item_mapping_table_path, index=False)
print("Saved item mapping table to:", item_mapping_table_path)

user_item1 = pd.merge(interaction, item_final, how='left', on = 'asin')
user_item2 = user_item1.dropna(axis=0)

user_item2.sort_values(by=["reviewerID","unixReviewTime"],inplace=True,ascending=[True,True])
user_click_num = pd.DataFrame(user_item2.groupby(user_item2['reviewerID']).count())
click_num=user_click_num.reset_index()[['reviewerID','asin']].rename(columns={'asin':'click_num'})
item_data6 = pd.merge(user_item2, click_num, how='left', on = 'reviewerID')
item_data7 = item_data6[item_data6['click_num']>1]
data = item_data7[['reviewerID', 'asin', 'unixReviewTime', 'price_num', 'cate','also_buy', 'also_view', 'price_level']]


group_sess_min = pd.DataFrame(data['unixReviewTime'].groupby(data['reviewerID']).min())
group_min=group_sess_min.reset_index()[['reviewerID','unixReviewTime']].rename(columns={'unixReviewTime':'min'})

data1 = pd.merge(data, group_min, how='left', on = 'reviewerID')

def get_interval(self_t, min_t):
    results = self_t - min_t
    return results
data1['time_iterval'] = data1.apply(lambda row: get_interval(row['unixReviewTime'], row['min']), axis=1)
data2 = data1[data1['time_iterval']<time_interval]
user_click_num2 = pd.DataFrame(data2.groupby(data2['reviewerID']).count())
click_num2=user_click_num2.reset_index()[['reviewerID','asin']].rename(columns={'asin':'click_num'})
data3 = pd.merge(data2, click_num2, how='left', on = 'reviewerID')
data_all = data3[data3['click_num']>1]



data_all = data_all.rename(columns={'reviewerID':'sessionID','asin':'itemID', 'unixReviewTime':'time', 'price_num':'price', 'price_level':'priceLevel', 'cate':'category'})
data_all = data_all[['sessionID', 'itemID', 'time', 'price', 'priceLevel', 'category']]

reviewerID2sessionID = {}
asin2itemID = {}
category2categoryID = {}

sessionNum = 0
itemNum = 0
categoryNum = 0


for _, row in data_all.iterrows():
    if row['sessionID'] not in reviewerID2sessionID:
        sessionNum += 1
        reviewerID2sessionID[row['sessionID']] = sessionNum
    if row['itemID'] not in asin2itemID:
        itemNum += 1
        asin2itemID[row['itemID']] = itemNum
    if row['category'] not in category2categoryID:
        categoryNum += 1
        category2categoryID[row['category']] = categoryNum
print('#session: ', sessionNum)
print('&item: ', itemNum)
print('#category: ', categoryNum)

def reSession(reviewerID):
    if reviewerID in reviewerID2sessionID:
        return reviewerID2sessionID[reviewerID]
    else:
        print('session is not recorded')
        return 'none'
    
def reItem(asin):
    if asin in asin2itemID:
        return asin2itemID[asin]
    else:
        print('item is not recorded')
        return 'none'
    
def reCate(category):
    if category in category2categoryID:
        return category2categoryID[category]
    else:
        print('category is not recorded')
        return 'none'
    
def reBrand(brand):
    if brand in brand2brandID:
        return brand2brandID[brand]
    else:
        print('brand is not recorded')
        return 'none'

def priceInt(price):
    return int(price)

data_all['sessionID']=data_all.sessionID.map(reSession)
data_all['itemID']=data_all.itemID.map(reItem)
data_all['priceLevel']=data_all.priceLevel.map(priceInt)
data_all['category']=data_all.category.map(reCate)

item2price = {}
for _, row in data_all.iterrows():
    if row['itemID'] not in item2price:
        item2price[row['itemID']] = row['priceLevel']


data = data_all[['sessionID', 'itemID', 'priceLevel', 'category']]


item_inter_num = pd.DataFrame(data.groupby(data['itemID']).count())
item_inter_num = item_inter_num.reset_index()[['sessionID', 'itemID']]
item_num=item_inter_num.rename(columns={'sessionID':'item_num'})
data = pd.merge(data, item_num, how='left', on = 'itemID')
data = data[data['item_num']>9]
data = data[['sessionID', 'itemID', 'priceLevel', 'category']]

# dict (sessionID:[itemID,itemID])
sess_all = {}
# dict (sessionID:[priceLevel, priceLevel])
price_all = {}
# dict (sessionID:[cate, cate])
cate_all = {}
# dict (sessionID:[brand, brand])
for _, row in data.iterrows():
    sess_id = row['sessionID']
    item_id = row['itemID']
    price = row['priceLevel']
    cate = row['category']
    if sess_id in sess_all:
        sess_all[sess_id].append(item_id)
        price_all[sess_id].append(price)
        cate_all[sess_id].append(cate)
    else:
        sess_all[sess_id] = []
        sess_all[sess_id].append(item_id)
        price_all[sess_id] = []
        price_all[sess_id].append(price)
        cate_all[sess_id] = []
        cate_all[sess_id].append(cate)

sess_total = data['sessionID'].max()
split_num = int(sess_total/10*9)

tra_sess = dict() # dict(session_id:[item_id,item_id])
tes_sess = dict() 
tra_price = dict()# dict(session_id:[price,price])
tes_price = dict()
tra_cate = dict()# dict(session_id:[cate,cate])
tes_cate = dict()
for sess_temp in sess_all.keys():
    all_seqs = sess_all[sess_temp]
    all_price = price_all[sess_temp]
    all_cate = cate_all[sess_temp]
    if len(all_seqs) < 2:
        continue
    if len(all_seqs) > 20:
        all_seqs = all_seqs[:20]
        all_price = all_price[:20]
        all_cate = all_cate[:20]
        
        
    if int(sess_temp) < split_num:
        tra_sess[sess_temp] = all_seqs
        tra_price[sess_temp] = all_price
        tra_cate[sess_temp] = all_cate
    else:
        tes_sess[sess_temp] = all_seqs
        tes_price[sess_temp] = all_price
        tes_cate[sess_temp] = all_cate

item_dict = {} #dict(old_itemID: new_itemID)
cate_dict = {} #dict(old_cate: new_cate)
price_dict = {} #dict(old_price: new_price)

item_price = {} #dict[new_itemID: priceLevel]
item_cate = {} #dict[new_itemID: cate]


# tra_sess tra_price tra_cate
# Convert training sessions to sequences and renumber items to start from 1
def obtian_tra():
    train_seqs = []
    train_price = []
    train_cate = []
    train_brand = []
    item_ctr = 1
    price_ctr = 1
    cate_ctr = 1
    for s in tra_sess:
        seq = tra_sess[s]
        price_seq = tra_price[s]
        cate_seq = tra_cate[s]
        outseq = []
        pri_outseq = []
        cate_outseq = []
        for i, p, c in zip(seq, price_seq, cate_seq):
            if i in item_dict:
                outseq += [item_dict[i]]
            else:
                outseq += [item_ctr]
                item_dict[i] = item_ctr
                item_ctr += 1
            if p in price_dict:
                pri_outseq += [price_dict[p]]
            else:
                pri_outseq += [price_ctr]
                price_dict[p]= price_ctr
                price_ctr += 1
            if c in cate_dict:
                cate_outseq += [cate_dict[c]]
            else:
                cate_outseq += [cate_ctr]
                cate_dict[c] = cate_ctr
                cate_ctr += 1
        if len(outseq) < 2:  # Doesn't occur
            print('session length is 1')
            continue
        train_seqs += [outseq]
        train_price += [pri_outseq]
        train_cate += [cate_outseq]
    print("#train_session",len(train_seqs))
    print("#train_items",item_ctr-1)
    print("#train_price",price_ctr-1)
    print("#train_category",cate_ctr-1)
    return train_seqs, train_price, train_cate


# Convert test sessions to sequences, ignoring items that do not appear in training set
def obtian_tes():
    test_seqs = []
    test_price = []
    test_cate = []
    for s in tes_sess:
        outseq = []
        out_price = []
        out_cate = []
        for i, j, k in zip(tes_sess[s], tes_price[s], tes_cate[s]):
            if i in item_dict:
                outseq += [item_dict[i]]
                out_price += [price_dict[j]]
                out_cate += [cate_dict[k]]
        if len(outseq) < 2:
            print('obtain test session length is 1')
            continue
        test_seqs += [outseq]
        test_price += [out_price]
        test_cate += [out_cate]
    return test_seqs, test_price, test_cate

# 数据增强
def process_seqs(iseqs, iprice, icate):
    out_seqs = []
    out_price = []
    out_cate = []
    labs = []
    max_length = 19
    for seq, pri, cat in zip(iseqs, iprice, icate):
        for i in range(1, len(seq)):
            tar = seq[-i]
            labs += [tar]
            out_seqs += [seq[:-i]]
            out_price += [pri[:-i]]
            out_cate += [cat[:-i]]
    return out_seqs, out_price, out_cate, labs

# 无数据增强
def process_seqs_no(iseqs, iprice, icate):
    print("no data augment")
    out_seqs = []
    out_price = []
    out_cate = []
    labs = []
    max_length = 19
    for seq, pri, cat in zip(iseqs, iprice, icate):     
        labs += [seq[-1]]
        out_seqs += [seq[:-1]]
        out_price += [pri[:-1]]
        out_cate += [cat[:-1]]
    return out_seqs, out_price, out_cate, labs

tra_seqs, tra_pri, tra_cat  = obtian_tra()
tes_seqs, tes_pri, tes_cat  = obtian_tes()

tr_seqs, tr_pri, tr_cat, tr_labs = process_seqs_no(tra_seqs, tra_pri, tra_cat)
te_seqs, te_pri, te_cat, te_labs = process_seqs_no(tes_seqs, tes_pri, tes_cat)

print('train sequence: ',tr_seqs[:5])
print('train price: ',tr_pri[:5])
print('train category: ',tr_cat[:5])
print('train lab: ',tr_labs[:5])

# construct all matrics whose shape is similar as session-items [[],[]]
def tomatrix(all_seqs, all_pri, all_cate):

    price_item_dict = {}
    price_item = []

    price_category_dict = {}
    price_category = []


    category_item_dict = {}
    category_item = []


    # price-item dict -> {price_id:[1, 3, 4]}

    for s_seq, p_seq, c_seq in zip(all_seqs, all_pri, all_cate):
        for i_temp, p_temp, c_temp in zip(s_seq, p_seq, c_seq):
            if p_temp not in price_item_dict:
#                 print('price_new: ',p_temp)
                price_item_dict[p_temp] = []
            if p_temp not in price_category_dict:
                price_category_dict[p_temp] = []
            if c_temp not in category_item_dict:
#                 print('category_new: ',c_temp)
                category_item_dict[c_temp] = []
            price_item_dict[p_temp].append(i_temp)
            price_category_dict[p_temp].append(c_temp)
            category_item_dict[c_temp].append(i_temp)

    price_item_dict = dict(sorted(price_item_dict.items()))
    price_category_dict = dict(sorted(price_category_dict.items()))
    category_item_dict = dict(sorted(category_item_dict.items()))
    print("#price",len(price_item_dict))
    print("#category",len(category_item_dict))
    
    price_item = list(price_item_dict.values())
    price_category = list(price_category_dict.values())
    category_item = list(category_item_dict.values())
    return price_item, price_category, category_item

def data_masks(all_sessions):
#     coo_matrix
    indptr, indices, data = [], [], []
    indptr.append(0)
    for j in range(len(all_sessions)):
        session = np.unique(all_sessions[j]) 
        length = len(session)
        s = indptr[-1]
        indptr.append((s + length))
        for i in range(length):
            indices.append(session[i]-1)
            data.append(1)
    results = (data, indices, indptr)
    return results

tra_pi, tra_pc, tra_ci = tomatrix(tra_seqs+tes_seqs, tra_pri+tes_pri, tra_cat+tes_cat)

tra = (tr_seqs, tr_pri, data_masks(tr_seqs), data_masks(tr_pri), data_masks(tra_pi), data_masks(tra_pc), data_masks(tra_ci), tr_labs)
tes = (te_seqs, te_pri, data_masks(te_seqs), data_masks(te_pri), data_masks(tra_pi), data_masks(tra_pc), data_masks(tra_ci), te_labs)
all = 0
for seq in tra_seqs:
    all += len(seq)
for seq in tes_seqs:
    all += len(seq)
print('#interactions: ',all)
print('#session: ',(len(tra_seqs) + len(tes_seqs)))
print('sequence average length: ', all/(len(tra_seqs) + len(tes_seqs) * 1.0))


train_data_path = 'output/Amazon/' + datasets_name


if not os.path.exists(train_data_path):
    os.makedirs(train_data_path)
path_data_train = train_data_path + "/train.txt"
path_data_test = train_data_path + "/test.txt"

pickle.dump(tra, open(path_data_train, 'wb'))
pickle.dump(tes, open(path_data_test, 'wb'))

# Save mapping dictionaries
mapping_dicts_path = train_data_path + '/mapping_dicts.pkl'
with open(mapping_dicts_path, 'wb') as f:
    pickle.dump({
        'reviewerID2sessionID': reviewerID2sessionID,
        'asin2itemID': asin2itemID,
        'category2categoryID': category2categoryID,
        'item_dict': item_dict,
        'price_dict': price_dict,
        'cate_dict': cate_dict,
        'item2price': item2price
    }, f)
print("Saved mapping dictionaries to:", mapping_dicts_path)

# Crear índice invertido: asin → índices de secuencia de entrenamiento
asin_to_seqs = {}
item_dict_inv = {v: k for k, v in item_dict.items()}
asin_dict_inv = {v: k for k, v in asin2itemID.items()}
for idx, seq in enumerate(tr_seqs):
    for item_id in seq:
        try:
            asin = asin_dict_inv[item_dict_inv[item_id]]
            asin_to_seqs.setdefault(asin, []).append(idx)
        except KeyError:
            continue

# Cargar item_mapping_table.csv
item_mapping_path = os.path.join(train_data_path, 'item_mapping_table.csv')
item_df = pd.read_csv(item_mapping_path)

# Agregar columna con las secuencias de entrenamiento en las que aparece cada asin
item_df['train_seq_indices'] = item_df['asin'].map(lambda x: asin_to_seqs.get(x, []))

# Guardar el archivo actualizado
updated_mapping_path = os.path.join(train_data_path, 'item_mapping_table_with_sequences.csv')
item_df.to_csv(updated_mapping_path, index=False)
print("Saved extended item mapping table to:", updated_mapping_path)

# Guardar mapeo categoryID → categoría original
categoryID2name = {v: k for k, v in category2categoryID.items()}
category_df = pd.DataFrame([
    {'categoryID': cid, 'category': name}
    for cid, name in categoryID2name.items()
])
category_mapping_path = os.path.join(train_data_path, 'categoryID_to_category_mapping.csv')
category_df.to_csv(category_mapping_path, index=False)
print("Saved categoryID to category mapping to:", category_mapping_path)

# Guardar mapeo itemID → asin
itemID2asin = {v: k for k, v in asin2itemID.items()}
item_df_map = pd.DataFrame([
    {'itemID': item_id, 'asin': asin}
    for item_id, asin in itemID2asin.items()
])
itemid_asin_mapping_path = os.path.join(train_data_path, 'itemID_to_asin_mapping.csv')
item_df_map.to_csv(itemid_asin_mapping_path, index=False)
print("Saved itemID to asin mapping to:", itemid_asin_mapping_path)

# Guardar descripción de priceLevel: rango de precios por nivel
price_summary = item_final.groupby('price_level').agg(
    count=('price_num', 'count'),
    min_price=('price_num', 'min'),
    max_price=('price_num', 'max'),
    mean_price=('price_num', 'mean'),
    std_price=('price_num', 'std')
).reset_index().rename(columns={'price_level': 'priceLevel'})

price_summary_path = os.path.join(train_data_path, 'priceLevel_summary.csv')
price_summary.to_csv(price_summary_path, index=False)
print("Saved priceLevel summary to:", price_summary_path)


# Guardar sesiones estructuradas en CSV para depuración y visualización
session_export_df = pd.DataFrame({
    "sessionID": list(range(1, len(tra_seqs)+1)),
    "item_sequence": tra_seqs,
    "price_sequence": tra_pri,
    "category_sequence": tra_cat,
    "session_length": [len(seq) for seq in tra_seqs]
})
structured_sessions_path = os.path.join(train_data_path, 'structured_sessions.csv')
session_export_df.to_csv(structured_sessions_path, index=False)
print("Saved structured session sequences to:", structured_sessions_path)

# Guardar sesiones de test estructuradas en CSV para depuración y visualización
test_session_export_df = pd.DataFrame({
    "sessionID": list(range(len(tra_seqs)+1, len(tra_seqs) + len(tes_seqs) + 1)),
    "item_sequence": tes_seqs,
    "price_sequence": tes_pri,
    "category_sequence": tes_cat,
    "session_length": [len(seq) for seq in tes_seqs]
})
structured_test_sessions_path = os.path.join(train_data_path, 'structured_test_sessions.csv')
test_session_export_df.to_csv(structured_test_sessions_path, index=False)
print("Saved structured test session sequences to:", structured_test_sessions_path)

print("dataset: ", datasets_name)
print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
print("done")
